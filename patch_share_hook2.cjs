...
const hookPath = path.join(__dirname, 'src/hooks/useShareContent.ts');
let content = fs.readFileSync(hookPath, 'utf8');

const newContent = `import { useAppSettings } from '../contexts/AppSettingsContext';
...
export const useShareContent = () => {
  ...
  const shareContent = async ({
    ...
    try {
      const config = settings?.shareConfig || {
      ...
      const appUrl = config.appUrl || settings?.global_config?.appUrl || 'https://play.google.com/store/apps/details?id=com.haripathshala';
...
\${config.socialCaption || '🙏 Jai Siyaram'}\`;
  ...
  const copyToClipboard = async (docId: string, appUrl?: string): Promise<boolean> => {
    try {
      const config = settings?.shareConfig || { shareEnabled: true };
...
console.log("Patched useShareContent.ts");
